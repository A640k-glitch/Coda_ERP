...
function getUnreconciled(businessId) {
  ...
  const alreadyMatchedIds = new Set(
    tdb.prepare(`SELECT matched_journal_id FROM bank_transactions WHERE business_id = ? AND matched_journal_id IS NOT NULL`)
      .all(businessId)
...
function matchTransaction(businessId, userId, bankTxId, { journalEntryId, newTransaction }) {
  const tdb = new TenantDB(businessId);
  const btx = tdb.prepare(`SELECT * FROM bank_transactions WHERE id = ? AND business_id = ?`).get(bankTxId, businessId);
...
async function syncTransactions(businessId, accountId) {
  ...
  if (syncResult && syncResult.success && syncResult.data && syncResult.data.transactions) {
    ...
    const insertTx = tdb.transaction((txs) => {
      let count = 0;
      for (const tx of txs) {
