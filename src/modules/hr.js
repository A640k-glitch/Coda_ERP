...
function getEmployee(businessId, id) {
  const tdb = new TenantDB(businessId);
  return tdb.prepare('SELECT * FROM employees WHERE id = ? AND business_id = ?').get(id, businessId);
}
...
function listEmployees(businessId, { status, limit = 100, offset = 0 } = {}) {
  ...
  return tdb.prepare(`SELECT * FROM employees WHERE ${where.join(' AND ')} ORDER BY name LIMIT ? OFFSET ?`)
    .all(...params, limit, offset);
}

function deleteEmployee(businessId, id) {
  const tdb = new TenantDB(businessId);
  const r = tdb.prepare('DELETE FROM employees WHERE id = ? AND business_id = ?').run(id, businessId);
  return r.changes > 0;
}
...
function recordAttendance(businessId, { employee_id, date, check_in, check_out }) {
  ...
  const existing = tdb.prepare("SELECT * FROM attendance WHERE employee_id = ? AND date = ? AND check_out IS NULL ORDER BY created_at DESC LIMIT 1")
    .get(employee_id, day);
  if (existing && check_out) {
    tdb.prepare("UPDATE attendance SET check_out = ?, status = 'completed' WHERE id = ?").run(check_out, existing.id);
    return tdb.prepare('SELECT * FROM attendance WHERE id = ?').get(existing.id);
  }
  ...
  return tdb.prepare('SELECT * FROM attendance WHERE id = ?').get(id);
}
