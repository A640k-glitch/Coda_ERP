...
function createSubscription(businessId, tierName) {
  ...
  tdb.prepare("UPDATE businesses SET tier = ?, subscription_status = 'active' WHERE id = ?").run(tierName, businessId);
  return tdb.prepare('SELECT * FROM subscriptions WHERE id = ?').get(id);
}
...
function recordPayment(businessId, data) {
  ...
  return tdb.prepare('SELECT * FROM payments WHERE id = ?').get(id);
}
